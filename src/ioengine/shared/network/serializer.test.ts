...
const TestSchema = {
  1: {
    fields: ["a", "b"] as const,
    validator: (v: unknown): v is { a: number; b: string } =>
      typeof v === "object" &&
      v !== null &&
  ...
  2: {
    fields: [] as const,
    validator: (v: unknown): v is Record<string, never> =>
      typeof v === "object" && v !== null,
  },
  3: {
    fields: ["x", "y", "z"] as const,
    validator: (v: unknown): v is { x: number; y: number; z: number } =>
      typeof v === "object" &&
      v !== null &&
...
describe("Serializer", () => {
  ...
  test("serialize failing validator throws mentioning validation", () => {
    expect(() =>
      serializer.serialize(1, { a: "bad", b: 123 } as unknown as TestDataMap[1]),
    ).toThrow(/valid/i);
  });

  test("deserialize failing validator throws mentioning validation", () => {
    expect(() =>
      serializer.deserialize([1, "not-a-number", 99] as unknown as [
        1,
        ...unknown[],
      ]),
    ).toThrow(/valid/i);
