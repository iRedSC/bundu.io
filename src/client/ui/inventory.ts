...
const invItems: Item[] = [
  ...
  function holdinventory(newItems: Item[] = [], ) {
    for (const newItem of newItems) {
      const existingIndex = localInventoryOrder.findIndex(item => item.result === newItem.result);
  
      if (existingIndex !== -1) {
        localInventoryOrder[existingIndex] = { ...localInventoryOrder[existingIndex], ...newItem };
      } else {
  ...
  for(let item of inventory.slots) {
    if (item.hovering) {
      ...
      for (let i = localInventoryOrder.length - 1; i >= 0; i--) {
        ...
        if (!currentItem) {
          localInventoryOrder.push(localInventoryOrder.splice(i, 1)[0]);
        }
      }
