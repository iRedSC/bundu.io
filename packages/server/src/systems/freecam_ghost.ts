...
export class FreecamGhostSystem extends System<GameEventMap> {
    ...
    /**
     * Drop viewer tracking without sending deletes — client world was/will be
     * rebuilt (park / syncSession). Call before reconcileViewer on reconnect.
     */
    clearViewer(viewerId: number): void {
        this.loaded.delete(viewerId);
    }
    ...
    private broadcastPosition(ghost: GameObject): void {
        ...
        // `add` (not `set`): SetPosition is keyed by packet id only, so `set`
        // would clobber other ghosts' poses for the same viewer in one tick.
        for (const viewerId of this.viewersWith(ghost.id)) {
