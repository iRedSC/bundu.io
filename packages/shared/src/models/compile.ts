...
import { modelHasParts } from "./types";
...
const MODEL_FILTERS = new Set(["grayscale"]);
...
function resolveRawDefs(input: Record<string, unknown>): Map<string, RawDef> {
    ...
    const visit = (id: string): RawDef => {
        ...
        if (!raw) throw new Error(`Unknown model definition "${id}"`);
...
function compilePart(name: string, value: unknown, path: string): PartDef {
    ...
    return {
        ...
        spillover: optionalNumber(raw.spillover, `${path}.spillover`),
...
function compileDisplay(
    ...
    if (raw.visual !== undefined) {
        throw new Error(
            `${path}: "visual" was renamed to "model" — use model: <id>`
        );
    }
    ...
    const model = raw.model === undefined
        ? undefined
        : string(raw.model, `${path}.model`);
    if (texture !== undefined && model !== undefined) {
        throw new Error(`${path}: expected at most one of texture or model`);
    }
    const pose = {
        x: optionalNumber(raw.x, `${path}.x`),
        y: optionalNumber(raw.y, `${path}.y`),
    ...
    if (model !== undefined) return { ...pose, model };
    if (texture !== undefined) return { ...pose, texture };
    if (defaultTexture !== undefined) return { ...pose, texture: defaultTexture };
    // Empty display: consumer renders the owning model's own content.
    return pose;
}

function compileDisplays(
    value: unknown,
    path: string,
    defaultTexture?: string
): Record<string, ModelDisplay> {
    if (value === undefined) return {};
    const result: Record<string, ModelDisplay> = {};
    for (const [name, entry] of Object.entries(record(value, path))) {
        result[name] = compileDisplay(entry, `${path}.${name}`, defaultTexture);
    }
    return result;
}
...
function compileSlots(value: unknown, path: string, parts: Set<string>): Record<string, SlotDef> | undefined {
    ...
    for (const [name, slot] of Object.entries(record(value, path))) {
        ...
        if (raw.context !== undefined && raw.display === undefined) {
            throw new Error(
                `${path}.${name}: "context" was renamed to "display"`
            );
        }
        ...
        result[name] = {
            part,
            display,
            mirrorX: optionalBoolean(raw.mirrorX, `${path}.${name}.mirrorX`),
            scale: optionalNumber(raw.scale, `${path}.${name}.scale`),
        };
    }
    return result;
}
...
function compileOrderedParts(raw: RawDef, id: string): PartDef[] {
    if (raw.parts === undefined) return [];
...
function compileDef(raw: RawDef): ModelDef {
    if (raw.contexts !== undefined) {
        throw new Error(
            `${string(raw.id, "definition.id")}: "contexts" was renamed to "displays"`
        );
    }
    ...
    const parts = compileOrderedParts(raw, id);
    ...
    if (!abstract && !modelHasParts(base) && !texture) {
        const selfContained = Object.values(displays).every(
            (display) => display.texture !== undefined || display.model !== undefined
        );
        if (Object.keys(displays).length === 0 || !selfContained) {
            throw new Error(
                `${id}: concrete models need parts, a texture, or displays with texture/model`
            );
        }
    }
...
export function compileModelDefs(input: Record<string, unknown>): CompiledModelDefs {
    ...
    for (const def of defs.values()) {
        for (const [name, display] of Object.entries(def.displays)) {
            ...
            const target = defs.get(display.model);
            if (!target) {
                throw new Error(
                    `${def.id}.displays.${name}.model: unknown model "${display.model}"`
                );
            }
            if (target.abstract) {
                throw new Error(
                    `${def.id}.displays.${name}.model: "${display.model}" is abstract`
                );
            }
        }
    }
